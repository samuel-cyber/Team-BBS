...
try {
const { userId, userIds, title, message, type = 'general', meta = {} }
= req.body;
...
? [userId]
: [];
if (!targets.length) return res.status(400).json({ error: 'userId or
userIds required' });
if (!title || !message) return res.status(400).json({ error: 'title
...
// ════════════════════════════════════════════════════════════
app.get('/', (req, res) => res.json({ message: 'SUWE backend v4',
version: '4.0' }));
// ════════════════════════════════════════════════════════════
...
try {
// Use the stored function that joins ajo_members with users (security
definer)
...
return res.status(400).json({ success: false, error: 'BVN must be
exactly 11 digits.' });
}
...
name: 'Sandbox Verified User', dob: '01 Jan 1990', phoneHint:
'080•••••000' });
}
try {
const token = await getInterswitchToken();
const bvnRes = await
axios.get(`${ISW_BASE}/api/v1/identity/bvn/fulldetails/${bvn}`, {
headers: { 'Authorization': `Bearer ${token}`, 'Content-Type':
'application/json' },
validateStatus: () => true,
timeout: 15000
});
console.log('[BVN] status:', bvnRes.status, 'body:',
JSON.stringify(bvnRes.data));
...
? phone.slice(0,3) + '•'.repeat(Math.max(0, phone.length-6)) +
phone.slice(-3)
: '•••••••••';
return res.json({
success: true,
...
console.error('[BVN] error:', err.response?.status,
...
return res.status(400).json({ success: false, error: 'BVN verification
...
const { groupId, userId, amount, groupName, userEmail, userName,
cycleNumber } = req.body;
const amountNum = Number(amount) || 0;
if (!qtbConfigured()) {
// SANDBOX MODE --- tell frontend to show mock payment form
return res.json({ success: true, sandbox: true, amount: amountNum,
...
const txRef =
`SUWE-AJO-${groupId.slice(0,8)}-${userId.slice(0,8)}-${Date.now()}`;
const redirectUrl = `${process.env.FRONTEND_URL ||
'https://team-bbs-sandy.vercel.app'}/frontend/pages/ajo.html?payment=success&group=${groupId}`;
...
{ headers: { 'Authorization': `Bearer ${token}`, 'Content-Type':
'application/json' }, timeout: 15000 }
);
const payUrl = payRes.data.redirectUrl || payRes.data.paymentUrl ||
...
status: 'pending',
tx_ref: txRef,
due_date: new Date(new Date().getFullYear(), new Date().getMonth() + 1,
1).toISOString().split('T')[0]
});
return res.json({ success: true, paymentUrl: payUrl, txRef });
} catch (err) {
console.error('[payment initiation]', err.response?.data ||
err.message);
if (!IS_PROD) {
return res.json({ success: true, sandbox: true, amount: amountNum,
groupName, groupId, userId });
}
return res.status(500).json({ success: false, error: 'Payment
...
supabaseAdmin.from('ajo_payments').insert({
group_id: groupId,
user_id: userId,
...
message: `Your ajo contribution of ₦${Number(amount).toLocaleString()}
has been recorded (sandbox mode).`,
...
.from('ajo_groups').select('group_name').eq('id',
groupId).maybeSingle();
...
user_id: m.user_id,
title: '💳 Member Paid',
message: `${payerName} has paid their
₦${Number(amount).toLocaleString()} contribution to
"${groupName}".`,
type: 'payment_confirmed',
...
const newTier = newScore >= 80 ? 'Platinum' : newScore >= 60 ?
'Gold' : newScore >= 40 ? 'Silver' : 'Bronze';
...
try {
const body = req.body;
const txRef = body.transactionReference || body.txnRef ||
body.merchantReference || '';
const isOk = body.responseCode === '00' || body.responseCode ===
'000';
...
.from('ajo_payments').select('*').eq('tx_ref',
txRef).maybeSingle();
if (payment) {
...
title: '✅ Payment Confirmed',
message: `Your ajo contribution of
₦${Number(payment.amount).toLocaleString()} has been confirmed by
Interswitch.`,
type: 'payment_confirmed',
...
const { groupId, groupName, senderId, senderName, senderBCS,
...
message: `${senderName} (BCS: ${senderBCS}) invited you to join
"${groupName}". Open the Ajo page to accept or decline.`,
type: 'ajo_invitation',
read: false,
...
const { groupId, groupName, creatorId, creatorBCS, contributionAmount }
= req.body;
try {
...
message: `"${groupName}" is now open. Join before the 1st of next
month! Contribution:
₦${Number(contributionAmount).toLocaleString()}/month.`,
type: 'new_group',
read: false,
meta: { group_id: groupId, creator_bcs: creatorBCS }
}));
for (let i = 0; i < notifs.length; i += 100) {
await supabaseAdmin.from('notifications').insert(notifs.slice(i, i +
100));
}
}
return res.json({ success: true });
...
message: `${posterName} posted: "${preview}..." --- check the
Community feed.`,
type: 'new_post',
read: false,
...
if (group.status !== 'forming') throw new Error('Group no longer
accepting members.');
if ((group.current_members || 0) >= group.max_members) throw new
Error('Group is full.');
// Check user not already in 2 groups
const { data: existing } = await supabaseAdmin.from('ajo_members')
.select('id').eq('user_id', userId);
if ((existing || []).length >= 2) throw new Error('You are already
in 2 groups.');
// Check not already in this group
const { data: already } = await supabaseAdmin.from('ajo_members')
.select('id').eq('user_id', userId).eq('group_id',
groupId).maybeSingle();
...
.update({ status: 'accepted', recipient_id: userId }).eq('id',
invitationId);
}
...
message: `${userRow?.full_name || 'Someone'} (BCS:
${userRow?.bcs_score || 0}) joined "${group.group_name}".
${(group.current_members || 0) + 1} of ${group.max_members}
members.`,
type: 'member_joined',
read: false,
...
current_members, max_members), sender:sender_id(full_name, bcs_score)')
.eq('status', 'pending');
if (phone) {
query =
query.or(`recipient_id.eq.${userId},recipient_phone.eq.${phone}`);
} else {
...
system: `Parse Nigerian market trader transactions. Return ONLY valid
JSON no backticks.
{"type":"sale"|"inventory","item_name":"name","quantity":n,"unit":"bags"|"crates"|"pieces"|"kg"|"litres"|"bundles"|"units","selling_price":n,"cost_price":n,"confidence":"high"|"medium"|"low","summary":"one
line"}
k=1000, ₦=naira. Multiply per-unit by quantity for total.`,
messages: [{ role: 'user', content: `Parse: "${input}"` }]
});
const parsed =
JSON.parse(message.content[0].text.replace(/```json|```/g,
'').trim());
...
const totals = {};
salesData.forEach(s => {
if (!totals[s.item_name]) totals[s.item_name] = { qty:0, revenue:0,
profit:0 };
...
.map(([n,t]) => `${n}: ${t.qty} units,
₦${t.revenue.toLocaleString()} revenue, ₦${t.profit.toLocaleString()}
profit`)
.join('\\n');
const msg = await anthropic.messages.create({
model: 'claude-haiku-4-5-20251001', max_tokens: 1024,
system: `Nigerian market advisor. Return ONLY valid JSON no markdown:
{"headline":"6-word headline","advice":"2-3
sentences","priceAlert":"1 sentence","strategyTip":"1
sentence","emoji":"single
emoji","sentiment":"positive"|"neutral"|"warning","urgency":"high"|"medium"|"low"}`,
messages: [{ role:'user',
content:`Location:${userProfile.market_location}\\nCategories:${userProfile.product_categories}\\nSales:\\n${summary}\\nTotal:${salesData.length}`
}]
});
res.json({ success: true, advice:
JSON.parse(msg.content[0].text.replace(/```json|```/g,'').trim())
});
} catch (err) {
console.error('[market-advice]', err.message);
res.status(500).json({ success: false, error: 'Could not generate
advice.' });
}
});
...
// ════════════════════════════════════════════════════════════
//  NOTIFY NEW GROUP MEMBER  (called by ajo.html on join)
//  POST /api/notify-new-group-member
//  Body: { groupId, userId, userName, userBCS }
// ════════════════════════════════════════════════════════════
  ...
  try {
    ...
    if (members?.length) {
      const notifs = members.map(m => ({
        user_id: m.user_id,
        title:   '🎉 New Member Joined!',
        message: `${userName || 'Someone'} (BCS: ${userBCS || 0}) just joined "${group?.group_name || 'your group'}".`,
        type:    'member_joined',
        read:    false,
        meta:    { group_id: groupId, new_member_id: userId }
      }));
    ...
    console.error('[notify-new-group-member]', err.message);
    return res.status(500).json({ success: false, error: err.message });
  }
});
...
console.log(`ISW keys: ${iswKeysConfigured() ? '✅ configured' :
'⚠️ missing → sandbox mode'}`);
console.log(`QTB merchant: ${qtbConfigured() ? '✅ configured' :
'⚠️ missing → sandbox payments'}`);
});
// ════════════════════════════════════════════════════════════
...
if (HF_API_KEY) headers['Authorization'] = `Bearer
${HF_API_KEY}`;
const payload = {
inputs: prompt,
parameters: {
...
console.warn(`[HF] Model ${model} failed (${status ||
err.message}), trying next...`);
// 503 = model loading, 429 = rate limited → try next model
if (status !== 503 && status !== 429) throw err;
}
}
throw new Error('All HuggingFace models unavailable. Please try again
in a moment.');
}
/**
...
if (!message?.trim()) {
return res.status(400).json({ success: false, error: 'Message is
required.' });
}
...
.from('users')
.select('full_name, market_location, product_categories, bcs_score,
bcs_tier')
.eq('id', userId)
.maybeSingle();
if (u) {
userContext = `\\nUser profile: ${u.full_name || 'Trader'}, sells
${u.product_categories || 'various goods'} in ${u.market_location
|| 'Nigerian market'}, BCS score ${u.bcs_score || 'N/A'}
(${u.bcs_tier || 'N/A'} tier).`;
}
} catch (_) { /* non-fatal */ }
}
const systemPrompt = `You are SUWE Assistant, a helpful AI built for
Nigerian market traders and micro-business owners.
You help users with:
- Understanding their BCS (Business Credit Score) and how to improve it
- Ajo (rotating savings group) questions --- joining, payments, cycles,
invitations
...
${userContext}
Keep answers concise, friendly, and practical. Use Naira (₦) for
currency.
If asked something outside your scope, politely redirect to business
topics.
Do not make up specific prices or market data --- advise users to check
local rates.
Respond in plain text (no markdown headers or bullets unless listing
steps).`;
const fullHistory = [
...history.slice(-8), // keep last 8 turns to stay within context
limits
...
if (!userId) {
return res.status(400).json({ success: false, error: 'userId is
required.' });
}
try {
...
.from('sales')
.select('item_name, quantity, selling_price, cost_price, profit, unit,
created_at')
.eq('user_id', userId)
...
success: true,
insights: null,
message: `No sales data found for the selected period (${period}).
Start recording sales to get insights!`
});
}
// ── 3. Aggregate by item ─────────────────────────────────
const byItem = {};
...
.slice(0, 8)
.map(([name, d]) => {
const margin = d.revenue > 0 ? ((d.profit / d.revenue) *
100).toFixed(1) : '0.0';
return `${name}: ${d.qty} units sold, ₦${d.revenue.toLocaleString()}
revenue, ₦${d.profit.toLocaleString()} profit, ${margin}% margin`;
}).join('\\n');
...
.from('ajo_members')
.select('group_id, payment_status, paid_this_cycle,
ajo_groups(group_name, contribution_amount)')
.eq('user_id', userId);
const ajoContext = ajoMemberships?.length
? `Ajo memberships: ${ajoMemberships.length}. Monthly ajo obligations:
₦${
ajoMemberships.reduce((sum, m) => sum +
(Number(m.ajo_groups?.contribution_amount) || 0), 0).toLocaleString()
}.`
: 'Not currently in any ajo groups.';
// ── 5. Build prompt ──────────────────────────────────────
const systemPrompt = `You are a sharp Nigerian business analyst AI
helping market traders grow their businesses.
Analyze the trader's sales data and return ONLY a valid JSON object ---
...
"growthAction": "single most impactful action they can take this
week",
"bcsAdvice": "how their current trading pattern affects their BCS
score",
...
const userPrompt = `Trader: ${user?.full_name || 'Unknown'},
${user?.market_location || 'Nigerian market'}
Categories: ${user?.product_categories || 'general goods'}
BCS Score: ${user?.bcs_score || 'N/A'} (${user?.bcs_tier ||
'N/A'})
Period: last ${period}
Total transactions: ${totalTransactions}
Total revenue: ₦${totalRevenue.toLocaleString()}
Total profit: ₦${totalProfit.toLocaleString()}
Profit margin: ${totalRevenue > 0 ? ((totalProfit / totalRevenue) *
100).toFixed(1) : 0}%
Top products:
${topItems}
...
const prompt = buildChatPrompt(systemPrompt, [{ role: 'user',
content: userPrompt }]);
// ── 6. Call HF model ─────────────────────────────────────
...
.sort((a, b) => b[1].revenue - a[1].revenue)
.slice(0, 5)
.map(([name, d]) => ({ name, revenue: d.revenue, profit: d.profit,
qty: d.qty }))
};
...
if (!question?.trim()) {
return res.status(400).json({ success: false, error: 'Question is
required.' });
}
const contextGuides = {
ajo: 'Focus on ajo rotating savings: joining groups, contribution
cycles, payouts, invitations, and trust scores.',
bcs: 'Focus on BCS (Business Credit Score): what it is (0-100 scale),
Bronze/Silver/Gold/Platinum tiers, how to improve it through timely
payments and sales activity.',
sales: 'Focus on sales tracking, profit margins, pricing strategies,
and inventory management for Nigerian market traders.',
general: 'Answer broadly about the SUWE platform features: ajo savings,
BCS scoring, sales tracking, and community.'
};
const systemPrompt = `You are a helpful assistant for SUWE, a Nigerian
fintech platform for market traders.
${contextGuides[context] || contextGuides.general}
Answer in 2-4 sentences. Be direct, friendly, and practical. Use ₦ for
Naira.`;
const prompt = buildChatPrompt(systemPrompt, [{ role: 'user',
content: question }]);
try {
...
return res.status(500).json({ success: false, error: 'Could not answer.
Please try again.' });
}
});
// ════════════════════════════════════════════════════════════
