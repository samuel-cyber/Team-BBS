...
if (error) { console.warn('getVendorProfile:', error.message); return
null; }
return data;
} catch (err) { console.warn('getVendorProfile exception:',
...
// ── SALES ─────────────────────────────────────────────────────
export async function logSale(userId, { itemName, quantity, unit,
costPrice, sellPrice }) {
...
if (error) { console.warn('getThreeMonthSales:', error.message);
return []; }
return data || [];
}
...
export async function upsertInventoryItem(userId, itemName, quantity,
unit = 'units') {
...
if (!existing) return;
await upsertInventoryItem(userId, itemName, Math.max(0,
existing.quantity - Number(quantitySold)));
}
...
[]; }
return (data || []).filter(item => item.quantity <=
(item.reorder_level || 5));
}
export async function hasInventory(userId) {
const { data, error } = await
supabase.from('inventory').select('id').eq('user_id',
userId).limit(1);
if (error) { console.warn('hasInventory:', error.message); return
false; }
...
return groups.map(g => ({ ...g, membership: memberships.find(m =>
m.group_id === g.id) || {} }));
}
export async function getUserAjoGroupCount(userId) {
const { data, error } = await
supabase.from('ajo_members').select('id').eq('user_id', userId);
if (error) { console.warn('getUserAjoGroupCount:', error.message);
return 0; }
return (data || []).length;
}
/**
* Fetch all members of a group with profile data.
* Uses the backend (service role) so RLS doesn't block cross-user
reads.
* Falls back gracefully if backend is unreachable.
*/
export async function getGroupMembers(groupId, backendUrl =
'https://team-bbs-sandy.vercel.app') {
try {
const { data: { session } } = await supabase.auth.getSession();
const res = await
fetch(`${backendUrl}/api/group-members/${groupId}`, {
...
const { data, error } = await
supabase.from('ajo_members').select('*')
...
if (filters.maxContribution) query = query.lte('contribution_amount',
filters.maxContribution);
if (filters.minContribution) query = query.gte('contribution_amount',
...
[]; }
return (data || []).filter(g => (g.current_members || 0) <
g.max_members);
}
...
const near = (data || []).filter(g => (g.current_members || 0) <
g.max_members);
...
return (data || []).filter(g => (g.current_members || 0) <
g.max_members).slice(0, limit);
}
export async function createAjoGroup(userId, { groupName,
contributionAmount, maxMembers, visibility, category, state }) {
const max = Math.min(Math.max(Number(maxMembers) || 8, 3), 8);
const { data: group, error: gErr } = await
supabase.from('ajo_groups').insert({
...
supabase.from('ajo_groups').select('*').eq('id',
groupId).maybeSingle();
if (!group) throw new Error('Group not found.');
if ((group.current_members || 0) >= group.max_members) throw new
Error('This group is full.');
if (group.status === 'active') throw new Error('Cannot join a group
mid-cycle.');
const { score: bcs } = await computeBCS(userId, false);
...
await supabase.from('notifications').update({ read: true }).eq('id',
notifId);
}
// ── COMMUNITY ─────────────────────────────────────────────────
...
export async function createTradePost(userId, { postType, title,
description, location, whatsappNumber }) {
...
await supabase.from('users').update({ bcs_score: score, bcs_tier:
'Bronze' }).eq('id', userId);
...
const tier = total >= 80 ? 'Platinum' : total >= 60 ? 'Gold' :
total >= 40 ? 'Silver' : 'Bronze';
await supabase.from('users').update({ bcs_score: total, bcs_tier: tier
}).eq('id', userId);
...
/**
* Calculate correct contribution due date for a group.
* If user already paid this month → show next month's 1st.
* If not paid yet → show this/next month's 1st (whichever is
upcoming).
*/
export function getContributionDueInfo(group, userPayments) {
const now = new Date();
const thisMonth = `${now.getFullYear()}-${String(now.getMonth() +
1).padStart(2, '0')}`;
...
const pm = `${pd.getFullYear()}-${String(pd.getMonth() +
1).padStart(2, '0')}`;
return pm === thisMonth;
});
// If paid already → next month
const targetMonth = paidThisMonth
? new Date(now.getFullYear(), now.getMonth() + 2, 1)
: new Date(now.getFullYear(), now.getMonth() + 1, 1);
const daysLeft = Math.max(0, Math.ceil((targetMonth - now) / 86400000));
return {
label: targetMonth.toLocaleDateString('en-GB', { day: 'numeric',
month: 'long', year: 'numeric' }),
daysLeft,
paidAlready: paidThisMonth
};
}
### **frontend/pages/ajo.html**
